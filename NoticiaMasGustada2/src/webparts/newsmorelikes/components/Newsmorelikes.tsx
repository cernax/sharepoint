          ...
          success: data => {
            ...
            if(resp.length == 0)
            {
              ...
              for(var x = 0; x < resp.length;x++){
                ...
                likecount = parseInt(resp[x].Cells[6].Value);

                if( parseInt(resp[x].Cells[6].Value) > 0){

                  if(parseInt(resp[x].Cells[6].Value) > mayor )
                  {
                    debugger;
                    mayor = parseInt(resp[x].Cells[6].Value);

                    tit = resp[x].Cells[2].Value;
                    ruta = resp[x].Cells[4].Value;
                    var imgdefault = "https://euroamerica.sharepoint.com/sites/MundoEuronetDesa/PublishingImages/NOTICIAS/2020/Azul%20Casa%20Cambio%20de%20Domicilio%20Tarjeta%20(1).png";
...
const App = (props) => {
  ...
  return (
    <>
    <span role="heading" aria-level={2} style={{fontSize:"28px", color:"#c00518"}} >Noticia Más Gustada</span>
      {
        d == ""
          ?
